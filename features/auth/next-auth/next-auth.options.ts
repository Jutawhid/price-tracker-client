...
import { gatewayFetcher } from "@/features/helper";
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        const API_URL = `${process.env.NEXT_PUBLIC_GATEWAY_API_URL}${Api.Login}`;

        const res = await gatewayFetcher({
          method: "POST",
          url: API_URL,
          data: credentials,
        });
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (user && account?.provider === "google") {
        try {
          const API_URL = `${process.env.NEXT_PUBLIC_GATEWAY_API_URL}${Api.GoogleLogin}`;
